...
void NmsBucketAggregator::ensure_bucket(int64_t ts_ms){
...
void NmsBucketAggregator::on_quote(const QuoteDepth& q){
    ...
    if(_has_last_quote && _last_trading_day == q.trading_day){
        ...
        // 首次或跨日：将累计值计入当前桶，以保证“启动即有统计值”
        _cur.amount_sum += q.turnover;
        _cur.volume_sum += (int64_t)q.volume;
    }
    double mid = (q.bid_price>0 && q.ask_price>0)? (q.bid_price+q.ask_price)/2.0 : _cur.midprice_last;
